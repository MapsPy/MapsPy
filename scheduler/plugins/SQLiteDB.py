...
CREATE_PROCESS_NODES_TABLE_STR = 'CREATE TABLE IF NOT EXISTS ProcessNodes(Id INTEGER PRIMARY KEY, ComputerName TEXT, NumThreads INTEGER, Status TEXT, Heartbeat TIMESTAMP);'
...
SELECT_ALL_JOBS = 'SELECT DataPath, ProcMask, Version, DetectorElements, MaxFilesToProc, MaxLineToProc, QuickAndDirty, XRF_Bin, NNLS, XANES_Scan, DetectorToStartWith, BeamLine, Standards FROM Jobs'
...
if __name__ == '__main__':
	import datetime
	proc_node = { 'ComputerName':'Comp1', 'NumThreads':1, 'Status':'idle', 'Heartbeat':datetime.datetime.now()}
	proc_node2 = { 'ComputerName':'Comp2', 'NumThreads':2, 'Status':'idle', 'Heartbeat':datetime.datetime.now()}
